...
public class MailInitializer {
    ...
    private static Credential getCredentials(final NetHttpTransport HTTP_TRANSPORT) throws IOException, Exception {
        // Load client secrets.
        InputStream in = MailInitializer.class.getResourceAsStream(CREDENTIALS_FILE_PATH);
        if (in == null) {
     ...
     * @throws IOException
     */
    public static List<MetroReceipt> getAllMetroReceipts(Gmail service, String userId, String query) throws IOException {
        ...
        List<MetroReceipt> allMetroReceipts = new ArrayList<MetroReceipt>();
        ...
        MetroReceipt receipt;
        SimpleDateFormat parserSDF = new SimpleDateFormat("MMM d, yyyy HH:mm:ss zzz");

        System.out.println("Number of receipts: "+messages.size());
        ...
        for(Message message : messages) {
            ...
            receipt = MetroReceipt.parse(m);


            Date date = new Date(m.getInternalDate());
            String strDate = parserSDF.format(date);

            //only add receipts that were single rides ($1.25)
            if(receipt != null && receipt.getCost() == 1.25) {
                allMetroReceipts.add(receipt);
            }
        }
        System.out.println("");

        return allMetroReceipts;
    }
